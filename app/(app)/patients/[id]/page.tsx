...
export default function PatientDetailPage() {
  ...
  const [secondaryInsurance, setSecondaryInsurance] = useState<InsuranceInfo | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await api.get<SingleResponse<ApiPatient & { insurance?: ApiInsurance[] }>>(`/api/patients/${params.id}`);
        if (!cancelled) {
          const mapped = mapApiPatientToPatient(res.data);
          // Map primary insurance from ordinal 1
          const insurances = res.data.insurance ?? [];
          const primary = insurances.find(i => i.ordinal === 1);
          const secondary = insurances.find(i => i.ordinal === 2);
          if (primary) mapped.primaryInsurance = mapApiInsuranceToInfo(primary);
          if (secondary) setSecondaryInsurance(mapApiInsuranceToInfo(secondary));
          setPatient(mapped);
        }
      } catch {
...
/* ===================== Insurance Tab ===================== */
function InsuranceTab({ patient, secondaryInsurance }: { patient: Patient; secondaryInsurance?: InsuranceInfo }) {
  const pri = patient.primaryInsurance;
  const sec = secondaryInsurance;
