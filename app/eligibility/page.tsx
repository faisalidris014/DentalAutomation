...
import { searchPatients, getPatient, runEligibilityCheck } from '@/lib/mockApi';
import {
...
const historyData = [
  { id: 'h1', date: '2026-03-15', payer: 'Delta Dental PPO', status: 'active' },
  { id: 'h2', date: '2026-01-08', payer: 'Delta Dental PPO', status: 'active' },
  { id: 'h3', date: '2025-10-10', payer: 'Delta Dental PPO', status: 'active' },
];
...
export default function EligibilityPage() {
  ...
  async function handleSelectPatient(id: string) {
    ...
    setSearchQuery(pat ? `${pat.firstName} ${pat.lastName}` : '');
    setSuggestions([]);
    setLoading(false);
  }
  ...
  async function handleRunCheck() {
    ...
    try {
      const res = await runEligibilityCheck(selectedPatient.id);
      setResult(res);
    } finally {
      setChecking(false);
          ...
          <Card accentColor="var(--accent)">
            <div className="ins-panel">
              ...
              <button
                ...
                disabled={checking || !selectedPatient.primaryInsurance}
              >
                {checking ? (
                  <>
